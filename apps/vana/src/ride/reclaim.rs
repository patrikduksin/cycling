...
use crate::ride::log::SAMPLES_PER_BATCH;
use crate::ride::log::SECTORS;
use crate::ride::log::SLOTS;
...
mod tests {
    ...
    struct Fake {
        sectors: Vec<[u8; crate::ride::log::SECTOR_SIZE]>,
    ...
    impl Fake {
        fn full() -> Self {
            Self {
                sectors: vec![[0; crate::ride::log::SECTOR_SIZE]; SECTORS],
                fail_at: None,
                calls: vec![],
    ...
    fn scanner_recovers_surviving_suffix_after_interrupted_clear() {
        ...
        use crate::ride::log::SLOT_SIZE;
        use crate::ride::log::SLOTS_PER_SECTOR;
        use crate::ride::log::Scanner;
        use crate::ride::log::Source;
        use crate::ride::log::encode;
        let mut media = Fake {
            sectors: vec![[0xff; crate::ride::log::SECTOR_SIZE]; SECTORS],
            fail_at: None,
            calls: vec![],
            corrupt_readback: None,
        };
        ...
        .enumerate()
        {
            ...
            slot.0[SLOT_SIZE - 4..].copy_from_slice(&crate::ride::log::commit_word().0);
            assert_eq!(crate::ride::log::decode(&slot), Some(entry));
            let at = (SLOTS_PER_SECTOR + offset) * SLOT_SIZE;
            let sector = at / crate::ride::log::SECTOR_SIZE;
            let within = at % crate::ride::log::SECTOR_SIZE;
