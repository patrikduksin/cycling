...
use crate::sensors::ble_profile::CadenceReading;
use crate::sensors::ble_profile::CscMeasurement;
use crate::sensors::ble_profile::HeartRate;
use crate::sensors::ble_profile::Profile;

use device_api::ble_transport::Packet;
use device_api::ble_transport::Selection;
...
pub fn selection(profile: Profile, name: &[u8], address: Option<[u8; 6]>) -> Option<Selection> {
    ...
    Some(Selection {
        name: device_api::connectivity::Text::new(name)?,
        address,
        service,
        characteristic,
    })
...
pub struct Client {
    ...
    transport: device_api::ble_transport::Snapshot,
...
impl Client {
    pub fn new(profile: Profile) -> Self {
        Self {
            ...
            transport: device_api::ble_transport::Snapshot::default(),
            heart: None,
    ...
    pub fn update(&mut self, mut transport: device_api::ble_transport::Snapshot) {
        if let Some(previous) = self.after_connection {
            if transport.connections > previous {
                self.after_connection = None;
            } else if transport.link == device_api::ble_transport::Link::Connected {
                transport.link = device_api::ble_transport::Link::Off;
            }
        }
        if transport.connections != self.transport.connections
            || transport.link != device_api::ble_transport::Link::Connected
    ...
    pub fn packet(&mut self, packet: Packet) {
        if self.transport.link != device_api::ble_transport::Link::Connected
        ...
        match self.profile {
            ...
            Profile::HeartRate => match HeartRate::parse(data) {
                Some(value) => {
                    ...
                    self.heart = crate::sensors::ble_profile::usable_heart(value)
                        .map(|bpm| (bpm, packet.received_ms));
                }
    ...
    pub fn snapshot(&self, now: u64) -> crate::sensors::ble_profile::Snapshot {
        let (heart_bpm, heart_age_ms) = self
            .heart
            .map(|(value, at)| crate::sensors::ble_profile::fresh_u16(value, at, now))
            .unwrap_or((None, None));
        let (cadence_tenths, cadence_age_ms) = self.cadence.snapshot(now);
        let link = match self.transport.link {
            device_api::ble_transport::Link::Off | device_api::ble_transport::Link::Advertising => {
                crate::sensors::ble_profile::Link::Off
            }
            device_api::ble_transport::Link::Scanning => {
                crate::sensors::ble_profile::Link::Scanning
            }
            device_api::ble_transport::Link::Connecting => {
                crate::sensors::ble_profile::Link::Connecting
            }
            device_api::ble_transport::Link::Connected => {
                crate::sensors::ble_profile::Link::Connected
            }
            device_api::ble_transport::Link::Retrying => {
                crate::sensors::ble_profile::Link::Retrying
            }
            device_api::ble_transport::Link::Failed => crate::sensors::ble_profile::Link::Failed,
        };
        crate::sensors::ble_profile::Snapshot {
...
mod tests {
    use super::*;
    fn state(connection: u32, dropped: u32) -> device_api::ble_transport::Snapshot {
        device_api::ble_transport::Snapshot {
            link: device_api::ble_transport::Link::Connected,
    ...
    fn contact_freshness_invalid_and_disconnect_remain_distinct() {
        ...
        client.update(device_api::ble_transport::Snapshot {
            link: device_api::ble_transport::Link::Retrying,
            ..state(1, 0)
        });
