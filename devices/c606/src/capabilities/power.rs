...
use crate::drivers::power_transition::Gate;
use crate::drivers::power_transition::Transition;
use core::cell::RefCell;
use core::sync::atomic::AtomicBool;
use core::sync::atomic::AtomicU8;
use core::sync::atomic::Ordering;
use device_api::observation::Error;
use device_api::power::Capabilities;
...
use device_api::power::Support;
use device_api::power::WakeSources;
...
fn prepare(work: &mut Work, light: &mut Backlight, now: u64) {
    ...
    let requests = [
        (
            begin(&mut work.wifi, || crate::drivers::wifi::power_request(true)),
            Failure::Wifi,
        ),
        (
            begin(&mut work.ble, || {
                crate::drivers::bluetooth::power_request(true)
            }),
            Failure::Ble,
        ),
        (
    ...
    let radios = [
        (
            work.wifi,
            crate::drivers::wifi::power_status(),
            Failure::Wifi,
        ),
        (
            work.ble,
            crate::drivers::bluetooth::power_status(),
            Failure::Ble,
        ),
    ...
    if work.gnss && gnss.state == device_api::position_control::State::Failed {
        abort(Failure::Gnss, now);
        return;
    }
    if work.sound && sound.state == device_api::sound::State::Failed {
    ...
    if !radios
        ...
        || !work.gnss
        || gnss.state != device_api::position_control::State::SilenceObserved
        || !work.sound
        || sound.state != device_api::sound::State::Elapsed
        || !ACCESS.drained()
    {
        return;
    }
    ...
    if status().operation == Some(Operation::Sleep) {
        ...
        let observed = crate::drivers::sleep::enter(|| {
        ...
        if let Ok(observed) = observed {
            firmware_services::network_time::sleep_elapsed(
        ...
        let slept = observed.is_ok_and(|o| o.outcome == crate::drivers::sleep::Outcome::TimerWake);
        let uncertain = observed.is_ok_and(|o| {
            matches!(
                o.outcome,
                crate::drivers::sleep::Outcome::TimedOut
                    | crate::drivers::sleep::Outcome::UnexpectedWake
            )
        });
        TRANSITION.lock(|t| {
            if uncertain {
    ...
    let result =
        crate::drivers::companion_uart::send(&crate::drivers::power_transition::shutdown_frame());
...
fn recover(work: &mut Work, light: &mut Backlight, now: u64) {
    ...
    if !work.restoring {
        ...
        work.restore_failed |= work.wifi && crate::drivers::wifi::power_request(false).is_err();
        work.restore_failed |= work.ble && crate::drivers::bluetooth::power_request(false).is_err();
    ...
    let recovery = crate::drivers::power_transition::recovery_status(
        [
            work.wifi.then(crate::drivers::wifi::power_status),
            work.ble.then(crate::drivers::bluetooth::power_status),
    ...
    if work.restore_failed || recovery == PeripheralState::Failed {
        ...
        && work.sleep_boundary.is_none_or(|at| {
            crate::drivers::power_transition::sensors_after(super::sensors::snapshot(now), at)
        })
        && (!work.charging
