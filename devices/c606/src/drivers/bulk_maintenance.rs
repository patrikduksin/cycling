...
use device_api::console::Console;
...
pub async fn run(mut usb: super::usb::Usb, bulk: crate::board::Bulk) {
    ...
    loop {
        ...
        if op == 5 {
            armed = None;
            let result = match crate::capabilities::power::ACCESS.enter() {
                Ok(_access) => media.recover(),
                Err(_) => Err(device_api::bulk::Error::Unavailable),
            };
        ...
        if op == 6 {
            ...
            let result = if start != 0 || count != 0 {
                Err(device_api::bulk::Error::Range)
            } else {
                match crate::capabilities::power::ACCESS.enter() {
        ...
        if op == 3 && supplied_token == token {
            ...
            let clock_result = match crate::capabilities::power::ACCESS.enter() {
