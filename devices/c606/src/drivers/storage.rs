...
impl<'d> Backend<'d> {
    ...
    fn address(
        ...
        device_api::storage::checked_range(capacity, offset, length, alignment)?;
...
impl OwnedFlash for Backend<'_> {
    type Error = Error;
    fn availability(&self, _region: Region) -> device_api::observation::Availability {
        if crate::capabilities::power::ACCESS.closed() {
