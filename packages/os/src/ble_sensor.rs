...
pub const fn ride_fields(
    ...
    if matches!(source, Some(crate::ride_log::Source::Live)) {
...
mod tests {
    ...
    fn only_live_ride_samples_receive_fresh_sensor_values() {
        ...
        assert_eq!(
            ride_fields(snapshot, Some(crate::ride_log::Source::Live)),
            (Some(72), Some(600))
        );
        assert_eq!(
            ride_fields(snapshot, Some(crate::ride_log::Source::Demo)),
            (None, None)
        );
