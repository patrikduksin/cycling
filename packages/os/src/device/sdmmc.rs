...
impl Reader<'_> {
    ...
    #[cfg(feature = "bulk-maintenance")]
    pub fn wide_read_mode(&mut self) -> Result<(), cycling_os::bulk::Error> {
        if self.failed {
            return Err(cycling_os::bulk::Error::Failed);
        }
        if self.report.kind != Kind::Mmc {
            return Err(cycling_os::bulk::Error::Unsupported);
        }
        if self.bus_width == 4 {
            return Ok(());
        }
        ...
        let result = (|| {
            if index == 18 && start + (output.len() / 512) as u64 == self.report.sectors {
                ...
                for (i, sector) in buffer.0[..output.len()].chunks_exact_mut(512).enumerate() {
                    let argument = cycling_os::bulk::address(
                        start + i as u64,
                        self.report.sectors,
    ...
    fn validate_owned(&mut self) -> Result<cycling_os::bulk::OwnedInfo, cycling_os::bulk::Error> {
        use cycling_os::bulk::{self, Read};
        let total = self.info()?.sectors;
        let mut mbr = [0; 512];
        self.read(0, &mut mbr)?;
        let layout = bulk::parse_mbr(&mbr, total)?;
        let mut marker = [0; 512];
        self.read(layout.custom_start, &mut marker)?;
        let info = bulk::parse_owned(&mbr, &marker, total)?;
        let binding = bulk::crc32(&mbr);
        if self
            .owned_binding
    ...
    fn range(&self, start: u64, bytes: usize) -> Result<u32, cycling_os::bulk::Error> {
        use cycling_os::bulk::{Error, address};
        if self.failed {
    ...
    fn complete(&mut self, result: Result<(), Error>) -> Result<(), cycling_os::bulk::Error> {
        result.map_err(|error| {
            self.failed = true;
            self.failures = self.failures.saturating_add(1);
            log::warn!(target: "mmc", "maintenance transfer failed error={:?}; recover required", error);
            cycling_os::bulk::Error::Failed
        })
    }
...
impl cycling_os::bulk::ReadWrite for Reader<'_> {
    ...
    fn owned_read(
        ...
        cycling_os::bulk::Read::read(self, info.address(sector)?, output)
    }
    fn owned_write(
        &mut self,
        sector: u64,
