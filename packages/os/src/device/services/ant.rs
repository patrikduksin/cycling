...
use cycling_os::ant::{Channels, Discovery, Identity, LinkState, Packet, Request, Snapshot};
use cycling_os::{capabilities::Error, power::PeripheralState};
...
struct Shared {
    ...
    suspending: bool,
    power_active: bool,
    restore: [Option<Identity>; cycling_os::ant::CHANNEL_CAPACITY],
    reconnect_sent: [bool; cycling_os::ant::CHANNEL_CAPACITY],
    power_deadline: u64,
}
static SHARED: Mutex<CriticalSectionRawMutex, RefCell<Shared>> = Mutex::new(RefCell::new(Shared {
    ...
    suspending: false,
    power_active: false,
    restore: [None; cycling_os::ant::CHANNEL_CAPACITY],
    reconnect_sent: [false; cycling_os::ant::CHANNEL_CAPACITY],
    power_deadline: 0,
}));
...
pub fn power_request(suspend: bool, now: u64) -> Result<(), Error> {
    SHARED.lock(|s| {
        ...
        s.power_deadline = now.saturating_add(cycling_os::ant::CONNECT_TIMEOUT_MS + 2_000);
                ...
                ScanOwnership::Active(_) => match self.state.stop_scan(now) {
                    Ok(request) => {
                        self.scan = ScanOwnership::Stopping(
                            now.saturating_add(cycling_os::ant::SCAN_STOP_TIMEOUT_MS),
                        );
...
pub fn receive(group: u8, payload: [u8; 8], now: u64) {
    if let Some(event) = crate::device::ant_protocol::decode(group, payload) {
        SHARED.lock(|s| {
            ...
            if matches!(event, cycling_os::ant::Event::ScanEnded) {
