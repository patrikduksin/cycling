...
use crate::sdk::ride_log::{SAMPLES_PER_BATCH, SECTORS, SLOTS, Sector, Status};
...
mod tests {
    ...
    struct Fake {
        sectors: Vec<[u8; crate::sdk::ride_log::SECTOR_SIZE]>,
    ...
    impl Fake {
        fn full() -> Self {
            Self {
                sectors: vec![[0; crate::sdk::ride_log::SECTOR_SIZE]; SECTORS],
                fail_at: None,
                calls: vec![],
    ...
    fn scanner_recovers_surviving_suffix_after_interrupted_clear() {
        use crate::sdk::ride_log::{
            Entry, Kind, SLOT_SIZE, SLOTS_PER_SECTOR, Scanner, Source, encode,
        };
        let mut media = Fake {
            sectors: vec![[0xff; crate::sdk::ride_log::SECTOR_SIZE]; SECTORS],
            fail_at: None,
            calls: vec![],
            corrupt_readback: None,
        };
        ...
        .enumerate()
        {
            ...
            slot.0[SLOT_SIZE - 4..].copy_from_slice(&crate::sdk::ride_log::commit_word().0);
            assert_eq!(crate::sdk::ride_log::decode(&slot), Some(entry));
            let at = (SLOTS_PER_SECTOR + offset) * SLOT_SIZE;
            let sector = at / crate::sdk::ride_log::SECTOR_SIZE;
            let within = at % crate::sdk::ride_log::SECTOR_SIZE;
