...
#[allow(dead_code)]
#[path = "../src/device/services/ant.rs"]
mod adapter;

#[test]
fn failed_uart_close_keeps_power_failed_without_replay() {
    use cycling_os::{ant::Identity, capabilities::AntOperation, power::PeripheralState};
    let peer = Identity {
        device_type: 120,
        device_number: 123,
        transmission_type: 1,
    ...
    *device::companion_uart::FAIL_NEXT.lock().unwrap() = true;
    adapter::tick(4);
    assert_eq!(adapter::power_status(), PeripheralState::Failed);
    assert_eq!(device::companion_uart::SENT.lock().unwrap().len(), 2);
    adapter::tick(20_000);
