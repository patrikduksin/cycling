...
impl AttributeData<'_> {
    pub(crate) fn value_len(&self) -> usize {
        match self {
            Self::Service { uuid, .. } => uuid.as_raw().len(),
            Self::IncludedService { uuid, .. } => 4 + uuid.as_ref().map_or(0, |uuid| uuid.len()),
            Self::ReadOnlyData { value, .. } => value.len(),
            Self::Data { len, .. } => usize::from(*len),
            Self::SmallData { len, .. } => usize::from(*len),
            Self::Declaration { uuid, .. } => 3 + uuid.as_raw().len(),
            Self::Cccd { .. } => 2,
        }
    }
