use device_api::position_control::*;
...
impl Controller {
    ...
    pub fn resume(&mut self, now: u64) -> Result<(), device_api::observation::Error> {
        if matches!(
            self.status.state,
    ...
    pub fn suspend(&mut self, now: u64) -> Result<(), device_api::observation::Error> {
