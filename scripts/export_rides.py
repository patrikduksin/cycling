...
def export_command(connection, command):
    reply = connection.terminal_command(f'EXPORT {command}')
    if reply['status'] != 'OK':
...
def info(connection):
    parts = export_command(connection, 'INFO')
...
def read_slot(connection, index):
    parts = export_command(connection, f'SLOT {index}')
