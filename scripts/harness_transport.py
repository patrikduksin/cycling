...
from usb import ROOT, UsbConnection, open_no_reset, terminal_reply
...
class Virtual:
    ...
    def __init__(self, directory, state_dir=None, sdk=False, harness=True):
        ...
        self.features = 'simulator' + (',debug-harness' if harness else '') + (',cycling' if sdk else '')
        self.request_id = secrets.randbelow(1 << 30) + 1
    ...
    def __enter__(self):
        ...
        self.process = subprocess.Popen(['cargo', 'run', '--quiet', '--locked', '--no-default-features',
