...
class UsbTests(unittest.TestCase):
    def test_reenumerated_tty_waits_before_first_accepted_submission(self):
        ...
        with (patch('usb.os.write', side_effect=[BlockingIOError(), len(request)]) as write,
              patch('usb.select.select', return_value=([123], [123], [])),
              patch('usb.os.read', return_value=response)):
    ...
    def test_short_submission_is_never_replayed(self):
        connection = UsbConnection('/dev/test', 'unused')
        connection.fd, connection.log = 123, io.BytesIO()
        with patch('usb.os.write', return_value=3) as write:
    ...
    def test_unwritable_timeout_reports_no_submission(self):
        connection = UsbConnection('/dev/test', 'unused')
        connection.fd, connection.log = 123, io.BytesIO()
        with patch('usb.os.write', side_effect=BlockingIOError()) as write:
    ...
    def test_wait_expiring_cannot_submit_a_mutation_after_its_deadline(self):
        connection = UsbConnection('/dev/test', 'unused')
        connection.fd, connection.log = 123, io.BytesIO()
        with (patch('usb.os.write', side_effect=[BlockingIOError(), len(b'CMD 1 SAVE\n')]) as write,
              patch('usb.time.monotonic', side_effect=[0, .5, 1.1]),
