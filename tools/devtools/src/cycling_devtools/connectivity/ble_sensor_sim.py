...
def main():
    ...
    parser.add_argument('--profile', choices=('heart', 'csc', 'both'),
                        default=os.environ.get('CYCLING_SIM_PROFILE', 'both'))
    args = parser.parse_args()
    if args.profile not in ('heart', 'csc', 'both'):
        parser.error('CYCLING_SIM_PROFILE must be heart, csc, or both')
    run(args.profile)
