...
from cycling_devtools.harness.runner import Runner, decode_chunk, events_for, png_rgb565, preflight, execute
from cycling_devtools.harness.transport import Real
...
class HarnessTests(unittest.TestCase):
    ...
    def test_failed_media_cleanup_does_not_skip_radio_or_preferences(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch('cycling_devtools.harness.runner.Virtual',return_value=fake),patch.object(Runner,'connectivity',side_effect=lambda radio,step: actions.append((radio,step['action']))),patch('sys.stdout',io.StringIO()):
    ...
    def test_connectivity_failure_still_restores_radio_and_preferences(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory)
            with patch('cycling_devtools.harness.runner.Virtual', return_value=fake), patch.object(Runner, 'delay') as delay, \
    ...
    def run_cleanup_case(self, fake, captures=()):
        ...
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory)
            with patch('cycling_devtools.harness.runner.Virtual', return_value=fake), \
                 patch('cycling_devtools.harness.runner.Capture', side_effect=captures), \
                 patch('cycling_devtools.harness.runner.time.monotonic', side_effect=lambda: fake.now), \
                 patch('sys.stdout', io.StringIO()):
                code = execute(args, scenario, output)
            report = json.loads((output / 'report.json').read_text())
    ...
    def test_recovery_does_not_replay_uncertain_restart_or_drop_lock(self):
        ...
        with patch('cycling_devtools.harness.transport.os.close'), patch('cycling_devtools.harness.transport.open_no_reset', return_value=6), \
             patch('cycling_devtools.harness.transport.rediscover', return_value='/dev/new'), patch.object(connection, 'delay'):
...
class WorkerIdentityTests(unittest.TestCase):
    def test_cancel_accepts_module_worker_and_rejects_reused_pid(self):
        from cycling_devtools.harness import runner
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            output = root / '.local/harness/runs/test'
            output.mkdir(parents=True)
            (output / 'pid').write_text('123')
            (output / 'report.json').write_text(json.dumps({'status': 'pass'}))
            command = b'\0'.join([b'python', b'-m', b'cycling_devtools.harness.runner',
                                  b'run', b'--output', str(output / 'result').encode()])
            with (patch.object(runner, 'ROOT', root),
                  patch('sys.argv', ['harness', 'cancel', 'test', '--seconds', '0']),
                  patch.object(Path, 'read_bytes', return_value=command) as cmdline,
                  patch.object(runner.os, 'kill') as kill,
                  patch('sys.stdout', io.StringIO())):
                self.assertEqual(runner.main(), 0)
                kill.assert_called_once_with(123, runner.signal.SIGTERM)
                kill.reset_mock()
                for invalid in (command.replace(b'cycling_devtools.harness.runner', b'unrelated'),
                                command.replace(b'/result', b'/result-other')):
                    cmdline.return_value = invalid
