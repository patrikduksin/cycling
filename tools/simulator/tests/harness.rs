...
use firmware_shell::harness::Operation;
use firmware_shell::shell::Shell;
use shell_simulator::devices::DisplayDevice;
use shell_simulator::devices::InputDevice;
use shell_simulator::devices::Memory;
use shell_simulator::devices::PowerDevice;
...
fn shell(w: usize, h: usize) -> (TestShell, DisplayDevice, InputDevice) {
    ...
    .unwrap();
    s.harness = firmware_shell::harness::State::new(7);
    s.harness.buffer = Some(firmware_shell::harness::CaptureBuffer::Owned(
        vec![0; firmware_shell::harness::FRAMES * w * h * 2].into_boxed_slice(),
    ));
    (s, d, i)
}
fn call(s: &mut TestShell, session: u64, op: Operation, now: u64) -> (&'static str, String) {
    ...
    let status = s.harness_command(
        firmware_shell::harness::Command {
            session,
            operation: op,
        },
        now,
...
fn event(s: &mut TestShell, session: u64, offset: u32, event: firmware_shell::harness::Event) {
    assert_eq!(
        call(
            s,
            session,
...
fn production_parser_bounds_and_typed_forms() {
    assert!(matches!(
        firmware_console::protocol::parse(b"CMD 42 HARNESS 99 INPUT ADD 1 20 DOWN 239 319")
            .unwrap()
            .command,
    ...
    for line in [
        ...
        assert!(firmware_console::protocol::parse(line.as_bytes()).is_err());
    }
    let mut lines = firmware_console::protocol::Lines::default();
    for _ in 0..firmware_console::protocol::MAX_LINE + 1 {
        assert!(lines.push(b'x').is_none());
    }
    assert_eq!(
        lines.push(b'\n'),
        Some(Err(firmware_console::protocol::Error::Overlong))
    );
...
fn timed_consumer_wake_physical_counters_and_expiry_cancel() {
    ...
    for (offset, event) in [
        (0, firmware_shell::harness::Event::Down(31, 23)),
        (2000, firmware_shell::harness::Event::Up),
    ] {
        assert_eq!(
            call(
                &mut s,
...
fn rejects_invalid_gestures_duplicate_execution_and_cancels_on_live_loss() {
    ...
    assert_eq!(
        call(
            ...
            Operation::InputAdd {
                sequence: 1,
                offset: 0,
                event: firmware_shell::harness::Event::Down(32, 23)
            },
            0
        )
        .0,
    ...
    assert_eq!(
        call(
            ...
            Operation::InputAdd {
                sequence: 1,
                offset: 0,
                event: firmware_shell::harness::Event::Move(1, 1)
            },
            0
        )
        .0,
    ...
    event(&mut s, id, 0, firmware_shell::harness::Event::Down(0, 0));
    assert_eq!(call(&mut s, id, Operation::InputRun(1), 0).0, "INVALID");
    event(&mut s, id, 500, firmware_shell::harness::Event::Up);
...
fn bounded_queue_overflow_and_session_replacement_reject_old_work() {
    ...
    let id = open(&mut s);
    call(&mut s, id, Operation::InputBegin(1), 0);
    for n in 0..firmware_shell::harness::EVENTS {
        event(
            &mut s,
            id,
            n as u32,
            firmware_shell::harness::Event::Button(0),
        );
    }
    assert_eq!(
        call(
            ...
            Operation::InputAdd {
                sequence: 1,
                offset: 100,
                event: firmware_shell::harness::Event::Button(0)
            },
            0
        )
        .0,
...
fn captures_exact_once_submitted_pixels_crc_geometry_and_failure() {
    for (w, h) in [(240, 320), (32, 24)] {
        ...
        assert!(meta.contains(&format!(
            "crc32={:08x}",
            firmware_shell::harness::crc32(&expected)
        )));
        let chunk = call(
            &mut s,
            id,
    ...
    assert_eq!(firmware_shell::harness::crc32(b"123456789"), 0xcbf43926);
}
#[test]
...
fn bounded_rle_transfer_round_trips_runs_raw_fallback_and_pixel_boundaries() {
    fn decode(text: &str) -> (String, usize, Vec<u8>, usize) {
        ...
        let encoded: Vec<u8> = hex
            ...
            .collect();
        assert!(encoded.len() <= firmware_shell::harness::ENCODED_BYTES);
    ...
    for (first_run, expected_encoding, expected_length) in [
        ...
        while offset < expected.len() {
            let requested = (expected.len() - offset).min(firmware_shell::harness::CHUNK);
            let (status, text) = call(
                &mut s,
                id,
                Operation::CaptureRead {
